...
query_file = "data/base/allquery.txt"
point_file = "data/point"
failed_file = "data/base/20161202.txt"
msglimit = 500000
sleep_limit = 10800 
...
def post_msgs():
	...
	with topic_obj.get_sync_producer() as producer:
		for word in query_words:
			try:
				res = producer.produce(word.strip().split("\t")[0])
				#logging.warning("append msg success:" + str(point) + " " + str(word.strip()))
				logging.error("append msg success:" + str(point) + " " + str(word.strip()))
			except:
				logging.error("append msg error:" + str(word.strip()))
				fail_fd.write(word)
			point = point + 1
